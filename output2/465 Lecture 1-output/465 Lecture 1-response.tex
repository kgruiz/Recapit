\documentclass[aspectratio=43]{beamer}

\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{tikz}
\usepackage{xcolor}
\usepackage{array}
\usepackage{enumitem}
\usepackage{tabularx}

\usetheme{Madrid}
\usecolortheme{default}

\setbeamertemplate{navigation symbols}{}

\setbeamertemplate{footline}{}

\title{}
\author{}
\date{}

\begin{document}
\begin{frame}
\frametitle{Homework}

\begin{itemize}
    \item There will be approximately 11 problem sets, posted on Canvas.
    \item Homework \#1 will be released on Tuesday, Jan 14. It will be due on Jan 20 at 11:59pm.
    \item Homework \underline{answers have to be justified}. No justification---no credit.
    \item Collaboration in small groups (up to four people total in a group) is allowed---but each case of collaboration has to be explicitly acknowledged on every homework submission.
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{Boolean functions}
\begin{block}{Problem}
How many Boolean functions in $k$ variables are there?
\end{block}

\begin{block}{Solution}
Each of the $k$ input bits can take value 0 or 1. There are $2^k$ possible input binary strings. A Boolean function can send each of them to either 0 or 1. So the total number of possibilities is $2^{2^k}$.
\end{block}

\begin{block}{Exercise}
Count microchips with $k$ binary inputs and $\ell$ binary outputs. Two microchips are considered identical if they compute the same function \{binary strings of length $k$\} $\rightarrow$ \{binary strings of length $\ell$\}.

For $\ell = 1$, you should recover the previous answer.
\end{block}

\end{frame}

\begin{frame}
\frametitle{Homework submission and grading}

Homework submission and grading will be done via \textcolor{blue}{Gradescope}.

On each problem set, only 5 problems will be graded.

Late homework will \underline{not} be accepted.

The lowest homework score will be dropped in the final calculation.

Gradescope has a built-in mechanism for regrade requests.

Any questions about the administrative aspects of the course?

\end{frame}

\begin{frame}
\frametitle{Proofs}

This is a \textcolor{blue}{proof-based} course. Students are expected to understand
rigorous mathematical proofs, and perhaps even more importantly,
supply their own proofs when asked.

Some specific prerequisites:
\begin{itemize}
    \item proofs by induction,
    \item proofs by contradiction,
    \item basic set theory terminology (intersection, union, etc.),
    \item basic function terminology (one-to-one, onto, composition, etc.).
\end{itemize}

If you are not familiar with Mathematical Induction, read either
[B\'ona, Chapter 2] or [Shahriari, Sections 1.1--1.2].

\end{frame}

\begin{frame}
\frametitle{Counting words}

An \textbf{alphabet} is a finite set of symbols (letters).

A \textbf{word} in an alphabet is a string (i.e., finite sequence) of letters.

\bigskip

\noindent\begin{minipage}{\linewidth}
\centering
\begin{tabular}{|l|}
\hline
Theorem \\
\hline
\end{tabular}
\end{minipage}

\bigskip

The number of $k$-letter words in an $n$-letter alphabet is $n^k$.

\bigskip

\begin{tabular}{llccccccccc}
$k = 2$ && AA & AB & AC & BA & BB & BC & CA & CB & CC \\
$n = 3$ \\
\hline
$k = 3$ && AAA & AAB & ABA & ABB & BAA & BAB & BBA & BBB \\
$n = 2$
\end{tabular}

\end{frame}

\begin{frame}
\frametitle{Subsets and binary strings}

\begin{block}{Theorem}
A $k$-element set has $2^k$ subsets.
\end{block}

$\boxed{k=3} \quad \emptyset \quad \{A\} \quad \{B\} \quad \{C\} \quad \{A,B\} \quad \{A,C\} \quad \{B,C\} \quad \{A,B,C\}$

\begin{block}{Proof}
The subsets of a $k$-element set are encoded by binary strings of length $k$, i.e., by $k$-letter words in a 2-letter alphabet:

$\emptyset \quad \{A\} \quad \{B\} \quad \{C\} \quad \{A,B\} \quad \{A,C\} \quad \{B,C\} \quad \{A,B,C\}$

$000 \quad 100 \quad 010 \quad 001 \quad 110 \quad 101 \quad 011 \quad 111$

The number of such words is $2^k$.
\end{block}

\end{frame}

\begin{frame}
    \frametitle{Content overview}

    This course introduces the fundamental notions, techniques, and theorems of \alert{enumerative combinatorics} and \alert{graph theory}.

    Level: undergraduate. Next level courses: \alert{Math 565}, \alert{Math 566}.

    Recommended texts (neither is required):
    \begin{itemize}
        \item M. B\'ona, \textit{A walk through combinatorics}, $4^{\text{th}}$ ed., World Scientific, 2016-17.
        \item S. Shahriari, \textit{Invitation to Combinatorics}, Cambridge University Press, 2022.
    \end{itemize}

    We will roughly cover Part II (\alert{Enumerative Combinatorics}) and Part III (\alert{Graph Theory}) of B\'ona's textbook, plus some other topics.

    We will not follow either textbook too closely. The lectures and the books will often provide slightly different approaches.

\end{frame}

\begin{frame}
\frametitle{Grading}

The grade will be based on:
\begin{itemize}
    \item homework (45\%),
    \item quizzes (15\%), and
    \item two 1.5-hour exams (20\% each).
\end{itemize}

This course will \textbf{not} be graded on a curve, i.e., there are not a set percentage of each grade to be given out.

Final grade cutoffs:
\begin{itemize}
    \item total score of 90\% guarantees the final grade of A or higher;
    \item total score of 80\% guarantees the final grade of B or higher;
    \item total score of 70\% guarantees the final grade of C or higher;
    \item total score of 60\% guarantees the final grade of D.
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{The Subtraction Principle}

The Subtraction Principle is used in situations where it is more practical to count the objects of interest together with some other objects, and then subtract the number of those extra objects.

\bigskip

\begin{block}{Problem}
How many ways are there to place 5 people named Amy, Bernie, Cory, Donald, and Elizabeth on the stage so that Amy and Bernie are \textit{not} adjacent to each other?
\end{block}

\bigskip

\begin{block}{Solution}
$5! - 4! \cdot 2 = 72$.
\end{block}

\end{frame}

\begin{frame}
\frametitle{Cyclic permutations}

\begin{block}{Problem}
How many ways are there to arrange $n$ distinct objects in a circle, up to rotation?
\end{block}

Example: seating a party of $n$ people around a round table.

\begin{center}
\begin{tikzpicture}[scale=0.7]
  \draw (0,0) circle (1cm);
  \node at (0,1.3) {B};
  \node at (1.3,0) {D};
  \node at (0,-1.3) {A};
  \node at (-1.3,0) {C};
\end{tikzpicture}
\quad
\begin{tikzpicture}[scale=0.7]
  \draw (0,0) circle (1cm);
  \node at (0,1.3) {C};
  \node at (1.3,0) {D};
  \node at (0,-1.3) {A};
  \node at (-1.3,0) {B};
\end{tikzpicture}
\quad
\begin{tikzpicture}[scale=0.7]
  \draw (0,0) circle (1cm);
  \node at (0,1.3) {A};
  \node at (1.3,0) {D};
  \node at (0,-1.3) {B};
  \node at (-1.3,0) {C};
\end{tikzpicture}
\quad
\begin{tikzpicture}[scale=0.7]
  \draw (0,0) circle (1cm);
  \node at (0,1.3) {C};
  \node at (1.3,0) {D};
  \node at (0,-1.3) {B};
  \node at (-1.3,0) {A};
\end{tikzpicture}
\quad
\begin{tikzpicture}[scale=0.7]
  \draw (0,0) circle (1cm);
  \node at (0,1.3) {A};
  \node at (1.3,0) {D};
  \node at (0,-1.3) {C};
  \node at (-1.3,0) {B};
\end{tikzpicture}
\quad
\begin{tikzpicture}[scale=0.7]
  \draw (0,0) circle (1cm);
  \node at (0,1.3) {B};
  \node at (1.3,0) {D};
  \node at (0,-1.3) {C};
  \node at (-1.3,0) {A};
\end{tikzpicture}
\end{center}

Answer: $(n - 1)!$.

\begin{block}{Exercise}
Given $n$ beads of different colors, how many circular necklaces are there that use all of these beads?
\end{block}

\end{frame}

\begin{frame}
\frametitle{Exams and quizzes}

\begin{block}{Exams (20\% each)}
There will be two exams, each covering one half of the course.
These exams will \textbf{not} be cumulative.
\end{block}

\begin{block}{Quizzes (15\% total)}
There will be three quizzes, administered via \textcolor{blue}{Canvas}:
\begin{itemize}
    \item at the end of next week, on \textcolor{blue}{Friday-Saturday, January 17–18};
    \item at the end of the week after, on \textcolor{blue}{Friday-Saturday, January 24–25};
    \item in the first week after spring break.
\end{itemize}

The goal of the first two quizzes is to help you decide whether this
course is a good match for you.
\end{block}

\end{frame}

\begin{frame}
\frametitle{Course management}
\begin{itemize}
    \item I am teaching Math 465 section 3 this Fall
    \item Location: East Hall B737
    \item Time: Tuesday and Thursday, 2:30-3:50 PM.
    \item The best way to communicate with me is by email.
    \item Most of the course management will occur on Canvas.
    \item \textcolor{blue}{Office hours} will be conducted in my office, EH 3827:
    \begin{itemize}
        \item Tuesday/Thursdasy 3:50–5:20 PM
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}
\begin{center}
\begin{minipage}{0.9\textwidth}
\begin{center}
\usebeamercolor[fg]{title}
\begin{beamercolorbox}[sep=8pt,center,rounded=true,shadow=true]{title}
{\usebeamerfont{title}\inserttitle\strut}
Math 465: Introduction to Combinatorics
\end{beamercolorbox}
\end{center}
\vspace{1cm}

Andrew Sack

\vspace{0.5cm}

asack@umich.edu

\vspace{1cm}

\hrulefill

\vspace{1cm}

These slides will be posted on Canvas.

\end{minipage}
\end{center}
\end{frame}

\begin{frame}
\frametitle{Permutations of subsets}
\begin{block}{Problem}
Given 34 people, how many ways are there for \textit{at most} 4 of them to wait in line?
\end{block}

\begin{block}{Solution}
Let us divide the set of possibilities into \textit{cases}:
\begin{itemize}
    \item The line has 0 people. There is 1 way this can happen.
    \item The line has 1 person. There are 34 ways this can happen.
    \item The line has 2 people. There are $34 \cdot 33 = 1122$ ways.
    \item The line has 3 people. There are $34 \cdot 33 \cdot 32 = 35904$ ways.
    \item The line has 4 people. There are $34 \cdot 33 \cdot 32 \cdot 31 = 1113024$ ways.
\end{itemize}
Hence the total number of ways is
\[
1 + 34 + 1122 + 35904 + 1113024 = \mathbf{1150085}.
\]
Note that we used both Addition and Multiplication Principles.
\end{block}
\end{frame}

\begin{frame}
\frametitle{The Division Principle}

The Division Principle is used in situations where each object of interest is counted several times (a fixed number).

\bigskip
\noindent\colorbox{lightgray}{
\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
\textbf{Problem}

How many diagonals are there in a convex $n$-gon?
\end{minipage}}

\vspace{0.5cm}

\begin{minipage}{0.45\textwidth}
\begin{center}
% Placeholder for the first polygon illustration
\textit{Polygon illustration with all diagonals}
\end{center}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\begin{center}
% Placeholder for the second polygon illustration
\textit{Polygon illustration with diagonals from one vertex}
\end{center}
\end{minipage}

\bigskip
\noindent\colorbox{lightgray}{
\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
\textbf{Solution}

For each of the $n$ vertices, there are $n-3$ diagonals connecting to it.

Each diagonal is counted twice. \textbf{Answer:} $n(n - 3)/2$.
\end{minipage}}

\end{frame}

\begin{frame}
\frametitle{Counting permutations}

\begin{block}{Theorem}
An $n$-element set has $n!$ permutations.
\end{block}

\begin{center}
\fbox{$n=3$} \quad 123 \quad 132 \quad 213 \quad 231 \quad 312 \quad 321
\end{center}

\begin{block}{Proof \#1: selecting the entries, left to right}
$n$ choices for the $1^{\text{st}}$ entry, then $n-1$ choices for the $2^{\text{nd}}$ entry, etc.
\end{block}

\begin{block}{Proof \#2: inserting the entries, in increasing order}
\begin{center}
\begin{tabular}{ccccccc}
 & 123 & 132 & 213 & 231 & 312 & 321 \\
$n = 3$ & & & & & & \\
\hline
$n = 4$ & 1234 & 1324 & 2134 & 2314 & 3124 & 3214 \\
 & 1243 & 1342 & 2143 & 2341 & 3142 & 3241 \\
 & 1423 & 1432 & 2413 & 2431 & 3412 & 3421 \\
 & \textcolor{red}{4}123 & \textcolor{red}{4}132 & \textcolor{red}{4}213 & \textcolor{red}{4}231 & \textcolor{red}{4}312 & \textcolor{red}{4}321 \\
\end{tabular}
\end{center}
\end{block}

\end{frame}

\begin{frame}
\frametitle{The Multiplication Principle, continued}

\begin{block}{Problem}
How many divisors does the number 600 have?
\end{block}

\begin{block}{Solution}
Since $600 = 2^3 \cdot 3 \cdot 5^2$, the answer is $4 \cdot 2 \cdot 3 = 24$.
\end{block}

\begin{block}{Problem}
How many rectangles are there in this picture?
\begin{center}
    \begin{tikzpicture}[scale=0.5]
    \draw (0,0) rectangle (3,3);
    \draw (1,0) -- (1,3);
    \draw (2,0) -- (2,3);
    \draw (0,1) -- (3,1);
    \draw (0,2) -- (3,2);
    \end{tikzpicture}
\end{center}
\end{block}

\begin{block}{Solution}
$(3 + 2 + 1) \times (3 + 2 + 1) = \textbf{36}$.
\end{block}

\end{frame}

\begin{frame}
\frametitle{The golden rule}

\begin{block}{The golden rule of counting}
Count everything \textcolor{blue}{exactly} once.
\end{block}

That is: count everything, and don't count anything more than once.

As we develop more sophisticated methods of counting, it will be important to make mental checks that everything is accounted for, and nothing is overcounted.

\end{frame}

\begin{frame}
\frametitle{Slide 18}

Error: Slide text content is None

\end{frame}\documentclass[aspectratio=43]{beamer}

\begin{frame}
\frametitle{Problem-solving challenges}
\begin{itemize}
    \item In past years, many students ended up dropping out of Math 465.
    \item Why? Much of the content is quite elementary.
    \item However, many problems are ``one of a kind,'' with no off-the-shelf algorithmic solution.
    \item The first two quizzes will give you some idea of where you stand.
    \item How do I prepare for quizzes/exams/homework?
    \item Use problems in the textbooks to practice!
    \item Also, come to office hours.
    \item Questions?
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{The Multiplication Principle (=Product Rule)}
\begin{block}{Problem}
Jimmy John's \textbf{Slim Roast Beef Sandwich} can be ordered:
\begin{itemize}
    \item on \textcolor{red}{French} bread, \textcolor{red}{wheat} bread, or \textcolor{red}{without} bread;
    \item with \textcolor{red}{regular}, \textcolor{red}{extra}, or \textcolor{red}{small} amount of roast beef;
    \item with \textcolor{red}{regular}, \textcolor{red}{extra}, \textcolor{red}{small}, or \textcolor{red}{no} amount of avocado spread;
    \item with \textcolor{red}{regular}, \textcolor{red}{extra}, \textcolor{red}{small}, or \textcolor{red}{no} amount of provolone cheese;
    \item with, \textcolor{red}{without}, or with \textcolor{red}{extra} Dijon mustard;
    \item with or \textcolor{red}{without} tomato.
\end{itemize}
How many ways are there to order the \textit{Slim Roast Beef Sandwich}?
\end{block}
\begin{block}{Solution}
$3 \cdot 3 \cdot 4 \cdot 4 \cdot 3 \cdot 2 = 864$.
\end{block}
\end{frame}

\begin{frame}
\frametitle{Unordered set partitions}
\begin{block}{Problem}
In how many ways can a 4-element set be partitioned into nonempty subsets (=blocks)?

Example: distributing students among Zoom breakout rooms.
\end{block}

\begin{block}{Solution}
There are 4 cases, depending on the number of blocks:
\begin{itemize}
    \item 1 block: $abcd$
    \item 2 blocks: $abc|d$ \ $abd|c$ \ $acd|b$ \ $bcd|a$ \ $ab|cd$ \ $ac|bd$ \ $ad|bc$
    \item 3 blocks: $ab|c|d$ \ $ac|b|d$ \ $ad|b|c$ \ $bc|a|d$ \ $bd|a|c$ \ $cd|a|b$
    \item 4 blocks: $a|b|c|d$
\end{itemize}
Total: $1+7+6+1=15$.

Exercise: Solve the analogous problem for a 5-element set.
\end{block}
\end{frame}

\begin{frame}
Basic counting principles [Chapter 3]

\vspace{0.5cm}
First half of the course: \textcolor{blue}{enumerative combinatorics}.

\vspace{0.3cm}
We will study the basic techniques of \textcolor{blue}{counting}, or \textcolor{blue}{enumeration}.

\vspace{0.5cm}
\begin{block}{Two versions of counting}
\begin{enumerate}
    \item Counting concrete objects;
    \item Counting choices/possibilities.
\end{enumerate}
\end{block}

\vspace{0.5cm}
\begin{block}{Examples}
\begin{enumerate}
    \item counting beer bottles in a party store;
    \item buying plane tickets for a given itinerary.
\end{enumerate}
\end{block}

\end{frame}
\end{document}